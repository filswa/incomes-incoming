...
export const calculateAverageIncome = (totalIncome, numOfIncomes) => ( totalIncome / numOfIncomes )

export const calculateLastMonthIncome = (incomes) => {
    const lastIncomeDate = new Date(Math.max.apply(null, incomes.map((item) => {
        return new Date(item.date);
      })));

    const lastMonthIncomes = incomes.filter((item) => {
        const itemDate = new Date(item.date);
        if (itemDate.getFullYear() === lastIncomeDate.getFullYear()
